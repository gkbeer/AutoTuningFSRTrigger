/*
  ...
  Auto Tuning FSR Trigger for Arduino Mini

  Author: Wing Tang Wong
  Modified by: Glenn Beer 4-24-14
  GitHub: https://github.com/WingTangWong/AutoTuningFSRTrigger/ATTINY85_TRINKET_FSR

  Description:

    This Arduino Sketch is meant to be installed on an Arduino Mini 05, which 
    uses a MEGA328P as the MCU. 
    ...
    =========   ==================================================================================
    R         - reset
    A0        - Used for FSR input.   analog.read(0) corresponds to A0
    A1        - Used for FSR input.
    A1        - Used for FSR input.   
    3         - Used for end stop output. HIGH by default, goes LOW when triggered.
    13        - Used as LED indicator. 
    VCC       - FTDI VCC input. 5V 
    Gnd       - FTDI gnd
    RX        - FTDI Serial
    TX        - FTDI Serial
    ========= ==================================================================================
...
// Pin assignments
int OUT = 3;          // OUTPUT pin
int LED = 13;         // LED pin
int A[3]  = {0,1,2};  // Analog pin numbers
int AD[3] = {0,1,2};  // Digital pin numbers correlating to the physical analog pins

// Various variables used for state and tracking calculations
unsigned long PRECALIBRATION=0;
unsigned long SEED = 80;
...
void setup() {  
  ...
  digitalWrite( LED, HIGH); // turn on led at start of calibration
  delay(3000); // wait 3 sec to begin
  do_calibration();
  // flash led on completion of calibration.
       int idx;
